...
public class CommandFramework implements CommandExecutor {
	...
	public CommandFramework(DailyReward plugin) {
		this.plugin = plugin;
		...
		for (int i = args.length; i >= 0; i--) {
			...
			if (commandMap.containsKey(cmdLabel)) {
				...
				if(command.isConsoleOnly() && (sender instanceof Player)) {
					sender.sendMessage(Color.colorize(config.getString("MESSAGE_CONSOLE_ONLY")));
					return true;
				}

				if (command.isAdminOnly() && !sender.hasPermission(config.getString("PERMISSION_ADMIN_ONLY"))) {
					sender.sendMessage(Color.colorize(config.getString("MESSAGE_NOT_ENOUGH_PERMISSIONS")));
					return true;
				}
				...
				if (command.isGameOnly() && !(sender instanceof Player)) {
					sender.sendMessage(Color.colorize(config.getString("MESSAGE_PLAYER_ONLY")));
					return true;
				}
